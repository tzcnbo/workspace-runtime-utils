...
生产/`pnpm start` 会启动 API Server，并从 `artifacts/api-portal/dist/public` 静态托管门户页面。API Server 会用 esbuild 打成自包含 `artifacts/api-server/dist/index.js`，部署运行时不依赖 `node_modules` 中的 express/cors/openai。
